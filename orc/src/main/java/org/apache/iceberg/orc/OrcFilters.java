...
public class OrcFilters {
    ...
    public static SearchArgument convert(Schema schema, Expression expr, boolean caseSensitive) {
        ConvertFilterToOrc toOrc = new ConvertFilterToOrc(schema, caseSensitive, SearchArgumentFactory.newBuilder());
        SearchArgument.Builder builder = visitDown(expr, toOrc);
        return builder.build();
    }

    public static SearchArgument.Builder visitDown(Expression expr, ConvertFilterToOrc visitor) {
        if (expr instanceof Predicate) {
            ...
            switch (expr.op()) {
                ...
                case NOT:
                    ...
                    return visitor.not(visitDown(not.child(), visitor));
                case AND:
                    And and = (And) expr;
                    visitor.getBuilder().startAnd();
                    return visitor.and(visitDown(and.left(), visitor), visitDown(and.right(), visitor));
                case OR:
                    Or or = (Or) expr;
                    visitor.getBuilder().startOr();
                    return visitor.or(visitDown(or.left(), visitor), visitDown(or.right(), visitor));
                default:
    ...
    public static class ConvertFilterToOrc extends ExpressionVisitor<SearchArgument.Builder> {
        private final Schema schema;
        private final boolean caseSensitive;
        private final SearchArgument.Builder builder;

        private ConvertFilterToOrc(Schema schema, boolean caseSensitive, SearchArgument.Builder builder) {
            this.schema = schema;
        ...
        @Override
        public Builder not(Builder result) {
            return builder.end();
        }

        @Override
        public Builder and(Builder leftResult, Builder rightResult) {
            return builder.end();
        }

        @Override
        public Builder or(Builder leftResult, Builder rightResult) {
            return  builder.end();
        }
        ...
        public <T> Builder predicate(BoundPredicate<T> pred) {
            if (!(pred.term() instanceof BoundReference)) {
                throw new UnsupportedOperationException("Cannot convert non-reference to Orc filter: " + pred.term());
            }
        ...
        private SearchArgument.Builder pred(Operation op, String column, PredicateLeaf.Type type, Object literal) {
            switch (op) {
                case IS_NULL:
